...
func (m DashboardModel) handleTick(msg TickMsg) (DashboardModel, tea.Cmd) {
	if !m.lock.Locked && m.lock.PassphraseHash != "" && time.Since(m.lock.LastInput) >= AutoLockAfter {
		m.lock.Locked = true
	...
	if m.timer.BreakActive {
		if time.Since(m.timer.BreakStart) >= BreakDuration {
	...
	if m.timer.ActiveSprint != nil {
		...
		if elapsed >= SprintDuration {
			if err := m.db.CompleteSprint(m.timer.ActiveSprint.ID); err != nil {
				m.setStatusError(fmt.Sprintf("Error completing sprint: %v", err))
				return m, tickCmd()
			}
			if err := m.db.MovePendingToBacklog(m.timer.ActiveSprint.ID); err != nil {
	...
	case tea.KeyMsg:
		...
		if msg.Type == tea.KeyEnter {
			...
			if m.lock.PassphraseHash == "" {
				...
				_, encrypted, _ := m.db.EncryptionStatus()
				encryptErr := error(nil)
				if !encrypted {
					if err := m.db.EncryptDatabase(entered); err != nil && err != database.ErrSQLCipherUnavailable {
						encryptErr = err
						if !m.db.DatabaseHasData() {
							if recErr := m.db.RecreateEncryptedDatabase(entered); recErr != nil {
				...
				if err := m.db.SetSetting("passphrase_hash", m.lock.PassphraseHash); err != nil {
				...
				} else if _, encrypted, _ := m.db.EncryptionStatus(); !encrypted {
...
func (m DashboardModel) handleModalState(msg tea.Msg) (DashboardModel, tea.Cmd) {
		...
		if msg.Type == tea.KeyEnter {
			if m.confirmingDelete {
				if m.confirmDeleteGoalID > 0 {
					if err := m.db.DeleteGoal(m.confirmDeleteGoalID); err != nil {
				...
				if err := m.db.ClearDatabase(); err != nil {
					m.err = err
				} else {
					wsID, wsErr := m.db.EnsureDefaultWorkspace()
					if wsErr != nil {
				...
				case 2:
					...
					encryptErr := error(nil)
					if encrypted {
						if err := m.db.RekeyDB(next); err != nil && err != database.ErrSQLCipherUnavailable {
							encryptErr = err
						...
						if err := m.db.EncryptDatabase(next); err != nil && err != database.ErrSQLCipherUnavailable {
							encryptErr = err
							if !m.db.DatabaseHasData() {
								if recErr := m.db.RecreateEncryptedDatabase(next); recErr != nil {
				...
				if strings.TrimSpace(text) != "" {
					...
					if err := m.db.AddJournalEntry(m.day.ID, activeWS.ID, sID, gID, text); err != nil {
				...
				if name != "" {
					newID, err := m.db.CreateWorkspace(name, strings.ToLower(name))
					if err == nil {
				...
				if num, err := strconv.Atoi(val); err == nil && num > 0 && num <= 8 {
					if err := m.db.BootstrapDay(m.pendingWorkspaceID, num); err != nil {
						...
						if dayID := m.db.CheckCurrentDay(); dayID > 0 {
							if day, err := m.db.GetDay(dayID); err == nil {
								m.day = day
							}
							m.refreshData(dayID)
						}
				...
				if err := m.db.SetGoalTags(m.editingGoalID, out); err != nil {
				...
				if len(m.themeNames) > 0 && m.themeCursor < len(m.themeNames) {
					...
					activeWS := m.workspaces[m.activeWorkspaceIdx]
					if err := m.db.UpdateWorkspaceTheme(activeWS.ID, name); err != nil {
				...
				if m.editingGoalID > 0 {
					if err := m.db.SetGoalDependencies(m.editingGoalID, deps); err != nil {
				...
				if m.editingGoalID > 0 {
					...
					switch rule {
					case "none":
						if err := m.db.UpdateGoalRecurrence(m.editingGoalID, ""); err != nil {
							m.setStatusError(fmt.Sprintf("Error saving recurrence: %v", err))
						}
					case "daily":
						if err := m.db.UpdateGoalRecurrence(m.editingGoalID, "daily"); err != nil {
							m.setStatusError(fmt.Sprintf("Error saving recurrence: %v", err))
						}
					case "weekly":
						var days []string
						...
						if len(days) == 0 {
							...
							if err := m.db.UpdateGoalRecurrence(m.editingGoalID, "weekly:"+strings.Join(days, ",")); err != nil {
								m.setStatusError(fmt.Sprintf("Error saving recurrence: %v", err))
							}
						}
					case "monthly":
						...
						default:
							...
							if err := m.db.UpdateGoalRecurrence(m.editingGoalID, rule); err != nil {
								m.setStatusError(fmt.Sprintf("Error saving recurrence: %v", err))
							}
						}
					}
				...
				if text != "" {
					if m.editingGoal {
						if err := m.db.EditGoal(m.editingGoalID, text); err != nil {
							m.setStatusError(fmt.Sprintf("Error updating goal: %v", err))
						}
					} else if m.editingGoalID > 0 {
						if err := m.db.AddSubtask(text, m.editingGoalID); err != nil {
							m.setStatusError(fmt.Sprintf("Error adding subtask: %v", err))
						} else {
							m.expandedState[m.editingGoalID] = true
						}
					} else {
						if err := m.db.AddGoal(m.workspaces[m.activeWorkspaceIdx].ID, text, m.sprints[m.focusedColIdx].ID); err != nil {
	...
	if m.changingPassphrase {
		...
		if keyMsg, ok := msg.(tea.KeyMsg); ok {
			...
			case "a":
				if m.confirmDeleteGoalID > 0 {
					if err := m.db.ArchiveGoal(m.confirmDeleteGoalID); err != nil {
		...
		if keyMsg, ok := msg.(tea.KeyMsg); ok {
			...
			case "u":
				if m.search.ArchiveOnly && len(m.search.Results) > 0 && m.search.Cursor < len(m.search.Results) {
					...
					if err := m.db.UnarchiveGoal(target.ID); err != nil {
						...
						m.search.Results, m.err = m.db.Search(query, m.workspaces[m.activeWorkspaceIdx].ID)
	...
	case tea.KeyMsg:
		...
		if len(msg.String()) == 1 && strings.Contains("012345678", msg.String()) {
			...
			if len(currentSprint.Goals) > m.focusedGoalIdx {
				...
				if found {
					if err := m.db.MoveGoal(goal.ID, targetID); err != nil {
...
func (m DashboardModel) handleNormalMode(msg tea.KeyMsg) (DashboardModel, tea.Cmd) {
	...
	case "T":
		if m.focusedColIdx < len(m.sprints) && len(m.sprints[m.focusedColIdx].Goals) > m.focusedGoalIdx {
			...
			if target.TaskActive {
				if err := m.db.PauseTaskTimer(target.ID); err != nil {
					m.setStatusError(fmt.Sprintf("Error pausing task timer: %v", err))
				} else {
					m.Message = "Task timer paused."
				}
			} else {
				if err := m.db.StartTaskTimer(target.ID); err != nil {
	...
	case "P":
		if m.focusedColIdx < len(m.sprints) && len(m.sprints[m.focusedColIdx].Goals) > m.focusedGoalIdx {
			...
			if err := m.db.UpdateGoalPriority(target.ID, next); err != nil {
	...
	case "ctrl+j":
		...
		return m, nil
	case "ctrl+e":
		path, err := ExportVault(m.db, m.lock.PassphraseHash)
		if err != nil {
	...
	case "A":
		if m.focusedColIdx < len(m.sprints) && len(m.sprints[m.focusedColIdx].Goals) > m.focusedGoalIdx {
			...
			if sprint.SprintNumber != -2 {
				if err := m.db.ArchiveGoal(sprint.Goals[m.focusedGoalIdx].ID); err != nil {
					m.setStatusError(fmt.Sprintf("Error archiving goal: %v", err))
				} else {
					m.invalidateGoalCache()
					m.refreshData(m.day.ID)
	...
	case "u":
		if m.focusedColIdx < len(m.sprints) && len(m.sprints[m.focusedColIdx].Goals) > m.focusedGoalIdx {
			sprint := m.sprints[m.focusedColIdx]
			if sprint.SprintNumber == -2 {
				if err := m.db.UnarchiveGoal(sprint.Goals[m.focusedGoalIdx].ID); err != nil {
					m.setStatusError(fmt.Sprintf("Error unarchiving goal: %v", err))
				} else {
					m.invalidateGoalCache()
					m.refreshData(m.day.ID)
	...
	case "b":
		if len(m.workspaces) > 0 {
			...
			if err := m.db.UpdateWorkspacePaneVisibility(activeWS.ID, activeWS.ShowBacklog, activeWS.ShowCompleted, activeWS.ShowArchived); err != nil {
	...
	case "D":
		if m.focusedColIdx < len(m.sprints) && len(m.sprints[m.focusedColIdx].Goals) > m.focusedGoalIdx {
			...
			deps, err := m.db.GetGoalDependencies(target.ID)
			if err != nil {
	...
	case "I":
		if len(m.workspaces) > 0 {
			...
			count, _, backlogFallback, err := ImportSeed(m.db, seedPath, activeWS.ID, m.day.ID)
			if err != nil {
	...
	case " ":
		if m.focusedColIdx < len(m.sprints) && len(m.sprints[m.focusedColIdx].Goals) > m.focusedGoalIdx {
			...
			blocked, err := m.db.IsGoalBlocked(goal.ID)
			if err != nil {
			...
			if canToggle {
				...
				if err := m.db.UpdateGoalStatus(goal.ID, newStatus); err != nil {
	...
	case "s":
		if !m.timer.BreakActive && m.focusedColIdx < len(m.sprints) {
			target := m.sprints[m.focusedColIdx]
			if target.SprintNumber > 0 {
				if m.timer.ActiveSprint != nil && m.timer.ActiveSprint.ID == target.ID {
					startedAt := time.Now()
					if m.timer.ActiveSprint.StartTime != nil {
						startedAt = *m.timer.ActiveSprint.StartTime
					}
					elapsed := int(time.Since(startedAt).Seconds()) + m.timer.ActiveSprint.ElapsedSeconds
					if err := m.db.PauseSprint(target.ID, int(elapsed)); err != nil {
						m.setStatusError(fmt.Sprintf("Error pausing sprint: %v", err))
					} else {
						m.refreshData(m.day.ID)
					}
				} else if m.timer.ActiveSprint == nil && (target.Status == "pending" || target.Status == "paused") {
					if err := m.db.StartSprint(target.ID); err != nil {
	...
	case "x":
		if m.timer.ActiveSprint != nil {
			if err := m.db.ResetSprint(m.timer.ActiveSprint.ID); err != nil {
	...
	case "+":
		activeWS := m.workspaces[m.activeWorkspaceIdx]
		if err := m.db.AppendSprint(m.day.ID, activeWS.ID); err != nil {
	...
	case "-":
		if len(m.workspaces) > 0 {
			activeWS := m.workspaces[m.activeWorkspaceIdx]
			if err := m.db.RemoveLastSprint(m.day.ID, activeWS.ID); err != nil {
	...
	case "v":
		...
		if len(m.workspaces) > 0 {
			activeWS := m.workspaces[m.activeWorkspaceIdx]
			if err := m.db.UpdateWorkspaceViewMode(activeWS.ID, m.viewMode); err != nil {
	...
	case "ctrl+r":
		activeWS := m.workspaces[m.activeWorkspaceIdx]
		path, err := GeneratePDFReport(m.db, m.day.ID, activeWS.ID)
		if err != nil {
		...
		return m, tea.Quit
	case "<":
		prevID, _, err := m.db.GetAdjacentDay(m.day.ID, -1)
		if err == nil {
	...
	case ">":
		nextID, _, err := m.db.GetAdjacentDay(m.day.ID, 1)
		if err == nil {
