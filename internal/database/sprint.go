...
func (d *Database) GetAdjacentDay(ctx context.Context, currentDayID int64, direction int) (int64, string, error) {
	...
	result, err := withDBContextResult(d, ctx, func(ctx context.Context) (dayResult, error) {
		var query string
		if direction < 0 {
		...
		return dayResult{id: id, date: date}, nil
	})
	if err != nil {
		return 0, "", err
	}
	return result.id, result.date, nil
}
...
func (d *Database) CheckCurrentDay(ctx context.Context) int64 {
	...
	id, err := withDBContextResult(d, ctx, func(ctx context.Context) (int64, error) {
		var id int64
...
func (d *Database) BootstrapDay(ctx context.Context, workspaceID int64, numSprints int) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "bootstrap", 0, err)
		}

		dateStr := time.Now().Format("2006-01-02")
		_, err = tx.ExecContext(ctx, "INSERT OR IGNORE INTO days (date) VALUES (?)", dateStr)
		if err != nil {
			return wrapErr(EntitySprint, "bootstrap", 0, rollbackWithLog(tx, fmt.Errorf("failed to ensure day: %w", err)))
		}

		var dayID int64
		err = tx.QueryRowContext(ctx, "SELECT id FROM days WHERE date = ?", dateStr).Scan(&dayID)
		if err != nil {
			return wrapErr(EntitySprint, "bootstrap", 0, rollbackWithLog(tx, err))
		}
		...
		for i := 1; i <= numSprints; i++ {
			...
			if err != nil {
				return wrapErr(EntitySprint, "bootstrap", 0, rollbackWithLog(tx, fmt.Errorf("failed to insert sprint %d: %w", i, err)))
			}
		}
...
func (d *Database) GetDay(ctx context.Context, id int64) (models.Day, error) {
	return withDBContextResult(d, ctx, func(ctx context.Context) (models.Day, error) {
		var day models.Day
		var dateStr string
...
func (d *Database) GetSprints(ctx context.Context, dayID int64, workspaceID int64) ([]models.Sprint, error) {
	return withDBContextResult(d, ctx, func(ctx context.Context) ([]models.Sprint, error) {
		...
		if err != nil {
			return nil, wrapErr(EntitySprint, "list", 0, err)
		}
		defer rows.Close()
...
func (d *Database) StartSprint(ctx context.Context, sprintID int64) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "start", sprintID, err)
		}
		return nil
	})
}
...
func (d *Database) PauseSprint(ctx context.Context, sprintID int64, elapsedSeconds int) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "pause", sprintID, err)
		}
		return nil
	})
}
...
func (d *Database) CompleteSprint(ctx context.Context, sprintID int64) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "complete", sprintID, err)
		}
		return nil
	})
}
...
func (d *Database) ResetSprint(ctx context.Context, sprintID int64) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "reset", sprintID, err)
		}
		return nil
	})
}
...
func (d *Database) AppendSprint(ctx context.Context, dayID int64, workspaceID int64) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "append", 0, err)
		}
		if lastSprintNum >= 8 {
			return wrapErr(EntitySprint, "append", 0, fmt.Errorf("max sprints reached (8)"))
		}
...
func (d *Database) RemoveLastSprint(ctx context.Context, dayID int64, workspaceID int64) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err := d.DB.QueryRowContext(ctx,
			...
			return wrapErr(EntitySprint, "remove", 0, err)
		}
		if count <= 1 {
			return wrapErr(EntitySprint, "remove", 0, fmt.Errorf("cannot remove last sprint"))
		}
		...
		if status == "active" || status == "paused" {
			return wrapErr(EntitySprint, "remove", sprintID, fmt.Errorf("cannot remove active sprint"))
		}
		if _, err := d.DB.ExecContext(ctx, "UPDATE goals SET sprint_id = NULL WHERE sprint_id = ?", sprintID); err != nil {
			return wrapErr(EntitySprint, "remove", sprintID, err)
		}
...
func (d *Database) GetSprintGoalCounts(ctx context.Context, sprintID int64) (int, int, error) {
	...
	result, err := withDBContextResult(d, ctx, func(ctx context.Context) (countResult, error) {
		var total int
		if err := d.DB.QueryRowContext(ctx, "SELECT COUNT(1) FROM goals WHERE sprint_id = ? AND status != 'archived'", sprintID).Scan(&total); err != nil {
			return countResult{}, wrapErr(EntitySprint, "counts", sprintID, err)
		}
		var completed int
		...
		return countResult{total: total, completed: completed}, nil
	})
	if err != nil {
		return 0, 0, err
	}
	return result.total, result.completed, nil
}
...
func (d *Database) MovePendingToBacklog(ctx context.Context, sprintID int64) error {
	return d.withDBContext(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return wrapErr(EntitySprint, "move pending", sprintID, err)
		}
		return nil
	})
}
